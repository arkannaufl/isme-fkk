...
class JadwalKuliahBesarController extends Controller
{
    ...
    public function store(Request $request, $kode)
    {
        ...
        if ($isSemesterAntara) {
            ...
            $validationRules['kelompok_besar_id'] = 'required|integer|min:1';
        }
        ...
        // Validasi bentrok
        $bentrokMessage = $this->validationService->validateConflict($data, 'kuliah_besar', null, $isSemesterAntara);
        if ($bentrokMessage) {
    ...
    public function update(Request $request, $kode, $id)
    {
        ...
        $bentrokMessage = $this->validationService->validateConflict($data, 'kuliah_besar', $id, $isSemesterAntara);
        if ($bentrokMessage) {
            return response()->json(['message' => $bentrokMessage], 422);
        }
    ...
    public function kelompokBesar(Request $request)
    {
        ...
        if ($jumlahMahasiswa > 0) {
            return response()->json([
                [
                    'id' => (int)$semester, // Gunakan semester sebagai ID untuk konsistensi
    ...
    public function importExcel(Request $request, $kode)
    {
        try {
            ...
            $antarBarisErrors = $this->validationService->validateAntarBarisExcel($excelData, 'kuliah_besar', $isSemesterAntara);
