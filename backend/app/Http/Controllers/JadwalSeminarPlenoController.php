...
class JadwalSeminarPlenoController extends Controller
{
    ...
    public function store(Request $request, $kode)
    {
        ...
        if (empty($koordinatorIds) || count($koordinatorIds) === 0) {
            return response()->json([
                'message' => 'Koordinator Dosen wajib diisi'
            ], 422);
        }
        ...
        if ($data['use_ruangan'] && $data['ruangan_id']) {
            $kapasitasMessage = $this->validationService->validateRoomCapacity(
                $dataForValidation,
                'seminar_pleno',
                false
            );
    ...
    public function import(Request $request, $kode)
    {
        try {
            ...
            $antarBarisErrors = $this->validationService->validateAntarBarisExcel($data['data'], 'seminar_pleno', false);
            ...
            foreach ($data['data'] as $index => $row) {
                ...
                if ($useRuangan) {
                    if (!$row['ruangan_id']) {
                        ...
                        if (!$ruangan) {
                            ...
                            $allDosenIds = array_unique(array_merge($koordinatorIds, $pengampuIds));
                            $rowDataForValidation = $row;
                            $rowDataForValidation['dosen_ids'] = $allDosenIds;
                            $rowDataForValidation['mata_kuliah_kode'] = $kode;

                            // Validasi kapasitas ruangan menggunakan service
                            $kapasitasMessage = $this->validationService->validateRoomCapacity(
                ...
                // Validasi bentrok menggunakan service
                $rowData = [
                    'mata_kuliah_kode' => $kode,
                    'tanggal' => $row['tanggal'],
