...
export default function PetaBlok() {
  ...
  // Query parameters untuk filter berdasarkan atribut (tanpa kode)
  const jenisFilter = searchParams.get('jenis');           // "Blok" atau "Non Blok"
  const blokKeFilter = searchParams.get('blok_ke');        // 1, 2, 3, 4
  const semesterFilter = searchParams.get('semester');     // 1-7
  const kurikulumFilter = searchParams.get('kurikulum');   // 2024, 2025, dst.
  ...
  useEffect(() => {
    // Redirect dari format spesifik ke format umum jika diperlukan
    // Logika sebelumnya dihapus karena kita sekarang mendukung kode dinamis
    // Kita hanya redirect jika parameternya benar-benar "format lama" yang numeric saja
    if (blok && !isNaN(parseInt(blok)) && blok !== "all" && blok.length < 3) {
      // Biarkan user di sini atau redirect ke 'all' karena kita tidak tahu kode pastinya tanpa lookup
      // Amannya kita redirect ke format tanpa kode spesifik agar menampilkan grid 'all'
  ...
  const [jadwalSeminarPleno, setJadwalSeminarPleno] = useState<any[]>([]);
  const [jadwalPersamaanPersepsi, setJadwalPersamaanPersepsi] = useState<any[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // 1. Ambil semua mata kuliah aktif dari API (sudah scoped ke tahun ajaran aktif)
      const allCoursesRes = await api.get('/mata-kuliah');
      const allCourses = Array.isArray(allCoursesRes.data) ? allCoursesRes.data : [];

      // 2. Filter mata kuliah berdasarkan Semester (Ganjil/Genap/Antara)
      let targetCourses: any[] = [];
      const semesterTargetGanjil = [1, 3, 5, 7];
      const semesterTargetGenap = [2, 4, 6];

      if (isAntara) {
        targetCourses = allCourses.filter((mk: any) =>
          mk.semester === 'Antara' ||
          mk.semester === 0 ||
          (typeof mk.semester === 'string' && mk.semester.toLowerCase() === 'antara')
        );
      } else if (isGanjil) {
        targetCourses = allCourses.filter((mk: any) => semesterTargetGanjil.includes(Number(mk.semester)));
      } else {
        targetCourses = allCourses.filter((mk: any) => semesterTargetGenap.includes(Number(mk.semester)));
      }

      // Filter berdasarkan query parameters (jenis, blok_ke, semester, kurikulum)
      // Ini menggantikan filter berdasarkan kode mata kuliah
      if (jenisFilter || blokKeFilter || semesterFilter || kurikulumFilter) {
        targetCourses = targetCourses.filter((mk: any) => {
          let match = true;
          if (jenisFilter) {
            match = match && (mk.jenis || '').toLowerCase() === jenisFilter.toLowerCase();
          }
          if (blokKeFilter) {
            match = match && mk.blok === parseInt(blokKeFilter);
          }
          if (semesterFilter) {
            match = match && String(mk.semester) === semesterFilter;
          }
          if (kurikulumFilter) {
            match = match && mk.kurikulum === parseInt(kurikulumFilter);
          }
          return match;
        });
      }
      // Backward compatibility: Filter by 'blok' param (specific course code)
      else if (blok && blok !== 'all' && blok !== 'NONBLOK' && !isAntara) {
        const specific = targetCourses.find((mk: any) => mk.kode === blok);
        if (specific) {
          targetCourses = [specific];
        } else {
          // Try to find in global list if not in target semester (user navigation flexibilty)
          const foundAnywhere = allCourses.find((mk: any) => mk.kode === blok);
          if (foundAnywhere) {
            targetCourses = [foundAnywhere];
          }
        }
      }

      // Filter for NONBLOK mode
      if (blok === 'NONBLOK') {
        targetCourses = targetCourses.filter((mk: any) => (mk.jenis || '').toLowerCase() === 'non blok');
      }

      setMataKuliahList(targetCourses);

      // 3. Fetch batch-data
      const batches = await Promise.all(
        targetCourses.map(async (mk: any) => {
          try {
            let endpoint = `/mata-kuliah/${mk.kode}/batch-data`;

            // Adjust endpoint based on type
            const isNonBlok = (mk.jenis || '').toLowerCase() === 'non blok';
            const isCsr = (mk.tipe_non_block || '').toLowerCase() === 'csr';

            if (isNonBlok) {
              if (isCsr) {
                endpoint = `/csr/${mk.kode}/batch-data`;
              } else {
                endpoint = `/non-blok-non-csr/${mk.kode}/batch-data`;
              }
            } else {
              endpoint = `/mata-kuliah/${mk.kode}/batch-data`;
            }

            const res = await api.get(endpoint);
            return { mk, data: res.data, success: true };
          } catch (e: any) {
            // Fallback retry
            if (e.response && e.response.status === 404) {
              try {
                const resRetry = await api.get(`/mata-kuliah/${mk.kode}/batch-data`);
                return { mk, data: resRetry.data, success: true };
              } catch (e2) {
                return { mk, data: null, success: false, error: e2 };
              }
            }
            return { mk, data: null, success: false, error: e };
          }
        })
      );
      ...
      const allSeminarPleno: any[] = [];
      const allPersamaanPersepsi: any[] = [];
      const allDates: { mulai?: string; akhir?: string }[] = [];

      batches.forEach(({ mk, data, success }) => {
        if (!success || !data) return;

        if (data.mata_kuliah) {
          allDates.push({
            mulai: data.mata_kuliah.tanggal_mulai,
            akhir: data.mata_kuliah.tanggal_akhir
          });
        }
        ...
        if (mk.semester === 'Antara') {
          ...
          semesterNum = Number(mk.semester) || 1;
        }
        const blokNum = (mk.jenis || '').toLowerCase() === 'blok' ? (mk.blok || 1) : 0;

        const addMeta = (item: any, type: string) => ({
          ...item,
          __semester: semesterNum,
          __blok: blokNum,
          __kode: mk.kode,
          __nama: mk.nama,
          __jenis: (mk.jenis || '').toUpperCase(),
          __jenis_jadwal: type
        });

        if (Array.isArray(data.jadwal_pbl)) data.jadwal_pbl.forEach((x: any) => allPbl.push(addMeta(x, 'PBL')));
        if (Array.isArray(data.jadwal_jurnal_reading)) data.jadwal_jurnal_reading.forEach((x: any) => allJurnal.push(addMeta(x, 'JURNAL')));
        if (Array.isArray(data.jadwal_praktikum)) data.jadwal_praktikum.forEach((x: any) => allPraktikum.push(addMeta(x, 'PRAKTIKUM')));
        if (Array.isArray(data.jadwal_agenda_khusus)) data.jadwal_agenda_khusus.forEach((x: any) => allAgendaKhusus.push(addMeta(x, 'AGENDA_KHUSUS')));

        if (Array.isArray(data.jadwal_kuliah_besar)) data.jadwal_kuliah_besar.forEach((x: any) => allKuliahBesar.push(addMeta(x, 'KULIAH_BESAR')));

        if (Array.isArray(data.jadwal_seminar_pleno)) data.jadwal_seminar_pleno.forEach((x: any) => allSeminarPleno.push(addMeta(x, 'SEMINAR_PLENO')));
        if (Array.isArray(data.jadwal_persamaan_persepsi)) data.jadwal_persamaan_persepsi.forEach((x: any) => allPersamaanPersepsi.push(addMeta(x, 'PERSAMAAN_PERSEPSI')));

        if (Array.isArray(data.jadwal_non_blok_non_csr)) {
          data.jadwal_non_blok_non_csr.forEach((x: any) => {
            let item = {
              ...x,
              materi: x.materi || x.agenda || "Non Blok",
              dosen_nama: x.dosen_names,
              ruangan_nama: x.ruangan?.nama
            };
            allKuliahBesar.push(addMeta(item, 'NON_BLOK_NON_CSR'));
          });
        }
        if (Array.isArray(data.jadwal_csr)) {
          data.jadwal_csr.forEach((x: any) => {
            let item = {
              ...x,
              materi: x.kategori?.nama || x.materi || "CSR",
              dosen_nama: x.dosen?.name,
              ruangan_nama: x.ruangan?.nama
            };
            allKuliahBesar.push(addMeta(item, 'NON_BLOK_CSR'));
          });
        }
      });
      ...
      setJadwalSeminarPleno(allSeminarPleno);
      setJadwalPersamaanPersepsi(allPersamaanPersepsi);
      ...
      if (!mulaiTimes.length) {
        [...allPbl, ...allJurnal, ...allKuliahBesar, ...allSeminarPleno, ...allPersamaanPersepsi, ...allAgendaKhusus, ...allPraktikum].forEach(item => {
          const t = new Date(item.tanggal || '').getTime();
          if (!isNaN(t)) { mulaiTimes.push(t); akhirTimes.push(t); }
        });
      }

      if (mulaiTimes.length && akhirTimes.length) {
        const minT = Math.min(...mulaiTimes);
        const maxT = Math.max(...akhirTimes);
        setMataKuliah({
          nama: isAntara ? "Jadwal Semester Antara" : `Jadwal Semester ${isGanjil ? 'Ganjil' : 'Genap'}`,
          semester: isGanjil ? 1 : 2,
          tanggal_mulai: new Date(minT).toISOString(),
          tanggal_akhir: new Date(maxT).toISOString()
        });
      } else {
        const now = new Date();
        ...
        setMataKuliah({
          ...
          tanggal_akhir: next.toISOString()
        });
      }
  ...
  }, [blok, isAntara, isGanjil, isGenap, tipe, jenisFilter, blokKeFilter, semesterFilter, kurikulumFilter]);
  ...
  const gridItems: CellItem[][][][] = useMemo(() => {
    ...
    // Seminar Pleno
    for (const sp of jadwalSeminarPleno) {
      ...
      const iso = toIsoDate(date);
      const jam = formatJamTanpaDetik(sp.jam_mulai || "");
      const jumlah = Number(sp.jumlah_sesi || 2);

      const topik = sp.topik || "";
      const kelompok = sp.kelompok_besar?.nama_kelompok || sp.kelompok_besar?.semester ? `Semester ${sp.kelompok_besar.semester}` : "";
      const dosen = sp.koordinator_names || sp.pengampu_names || "";
      const ruang = sp.ruangan?.nama || sp.ruangan_nama || "";
      const label = ["Seminar Pleno", topik || "Seminar Pleno"].join(" - ");
      const semEntry =
        sp.__semester != null
          ? Number(sp.__semester)
          : mataKuliah?.semester
            ? Number(mataKuliah.semester)
            : null;
      ...
      add(iso, jam, jumlah, label, "SEMINAR_PLENO", semEntry, detail);
    }

    // Persamaan Persepsi
    for (const pp of jadwalPersamaanPersepsi) {
      const date = new Date(pp.tanggal || "");
      if (isNaN(date.getTime())) {
        continue;
      }

      const iso = toIsoDate(date);
      const jam = formatJamTanpaDetik(pp.jam_mulai || "");
      const jumlah = Number(pp.jumlah_sesi || 2);

      const topik = pp.topik || "";
      const kelompok = "";
      const dosen = pp.koordinator_names || pp.pengampu_names || "";
      const ruang = pp.ruangan?.nama || pp.ruangan_nama || "";
      const label = ["Persamaan Persepsi", topik || "Persamaan Persepsi"].join(" - ");
      const semEntry =
        pp.__semester != null
          ? Number(pp.__semester)
          : mataKuliah?.semester
            ? Number(mataKuliah.semester)
            : null;
      ...
      add(iso, jam, jumlah, label, "PERSAMAAN_PERSEPSI", semEntry, detail);
    }
  ...
  const exportJadwalToExcel = async () => {
    try {
      ...
      jadwalSeminarPleno.forEach((item) => addSchedule(item, "SEMINAR_PLENO"));
      jadwalPersamaanPersepsi.forEach((item) => addSchedule(item, "PERSAMAAN_PERSEPSI"));
      ...
      link.href = url;
      link.download = `JadwalBlok_${isGanjil
        ? "Ganjil"
        : isGenap
          ? "Genap"
          : isAntara
  ...
  const exportJadwalToHTML = () => {
    ...
    let html = "<table>";
    html += "<thead><tr>";
    html += '<th rowspan="2">HARI / TANGGAL</th>';
    ...
    for (let h = 0; h < hariList.length; h++) {
      ...
      for (let s = 0; s < sesiHarian.length; s++) {
        ...
        if (s === 0) {
          html += `<td rowspan="${sesiHarian.length
            }"><div class="hari">${hariList[
              h
            ].hari.toUpperCase()}</div><div class="tanggal">${hariList[h].tanggal
            }</div></td>`;
        }
      ...
      html += `<tr><td colspan="${2 + semesterKolom.length
    ...
    a.href = url;
    a.download = `JadwalBlok_${blok === "all" ? "SemuaBlok" : blok
                          ...
                          semesterKolom.map((sem, semIdx) => (
                            ...
                            >
                              {(
                                ...
                                <div
                                  key={i}
                                  className={`mb-1.5 p-1.5 rounded-sm border-l-2 text-[10px] leading-3.5 font-medium ${it.jenis === "PBL"
                                      ...
                                      : it.jenis === "KULIAH_BESAR"
                                        ...
                                        : it.jenis === "AGENDA_KHUSUS"
                                          ...
                                          : it.jenis === "PRAKTIKUM"
                                            ...
                                            : it.jenis === "SEMINAR_PLENO"
                                              ? "border-teal-500 bg-teal-50/30 dark:bg-teal-900/20 dark:border-teal-400 text-teal-700 dark:text-teal-200"
                                              : it.jenis === "PERSAMAAN_PERSEPSI"
                                                ? "border-indigo-500 bg-indigo-50/30 dark:bg-indigo-900/20 dark:border-indigo-400 text-indigo-700 dark:text-indigo-200"
