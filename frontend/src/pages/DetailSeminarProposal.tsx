...
const DetailSeminarProposal = () => {
      ...
      if (id) {
        try {
          const [hasilResponse, penilaianResponse] = await Promise.all([
            api.get(`/hasil-seminar-proposal/jadwal/${id}`),
            api.get(`/penilaian-seminar-proposal/jadwal/${id}`),
          ]);
  ...
  const generatePDFForMahasiswa = async (
    ...
    try {
      ...
      if (jadwal.komentator_list) {
        ...
        komentatorResults.forEach(({ pengujiId, signature }: any) => {
          if (signature) {
            pengujiSignatures[pengujiId] = signature;
          }
        });
      ...
      doc.setFont("times", "normal");
      doc.text("Moderator/Ketua Penguji,", pageWidth / 2, yPos, { align: "center" });
    ...
    try {
      ...
      for (const mahasiswa of jadwal.mahasiswa_list) {
        ...
        if (pdfData) {
          ...
          const fileName = `Berita_Acara_Sempro_${sanitizedName}_${mahasiswa.nim}_${jadwal.tanggal.replace(/-/g, "")}.pdf`;
      ...
      a.href = url;
      a.download = `Berita_Acara_Sempro_${jadwal.tanggal.replace(/-/g, "")}.zip`;
  ...
  return (
    <>
      <PageMeta title="Detail Seminar Proposal | ISME" description="Detail Jadwal Seminar Proposal" />
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                Informasi lengkap jadwal seminar proposal
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {/* Komentator */}
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Komentator</p>
                {jadwal.komentator_list && jadwal.komentator_list.length > 0 ? (
                  <div className="space-y-2">
                    {jadwal.komentator_list.map((komentator, idx) => (
                      <div key={komentator.id || idx} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50">
                        <div className="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                          <span className="text-sm font-medium text-green-600 dark:text-green-400">
                            {komentator.name.charAt(0).toUpperCase()}
                          </span>
                        </div>
...
export default DetailSeminarProposal;
