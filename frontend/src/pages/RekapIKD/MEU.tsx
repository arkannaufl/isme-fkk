...
const MEU: React.FC = () => {
  ...
  // State untuk trigger re-render jika diperlukan
  ...
  const getIndicator = (pedoman: IKDPedoman): string => {
    ...
    // Jika item punya isi, cari parent yang tidak punya isi
    if (hasContent(pedoman)) {
      let currentNumber = match ? match[1] : null;
      if (!currentNumber) return "-";

      // Loop untuk mencari parent
      while (currentNumber) {
        // Cek di parentItemsMap dulu
        if (pedoman.parent_id && parentItemsMap.has(pedoman.parent_id)) {
          const parent = parentItemsMap.get(pedoman.parent_id);
