...
const SDM: React.FC = () => {
        ...
        Object.keys(newMap).forEach((key) => {
          if (!currentDeletedKeys.has(key)) {
            ...
            if (fileWithSkor) {
              ...
              filteredMap[key] = files.map((f) => ({
                ...f,
                is_saved: hasSavedFile,
              }));
            }

            // Jika ada file yang sudah disimpan, tambahkan ke savedKeys
            if (hasSavedFile) {
              // This line was part of the old logic for newSavedKeys, which is now removed.
              // It's safe to remove this line as newSavedKeys is no longer used.
            }
          }
        });
  ...
  const handleDeleteFile = async (buktiFisikId: number, fileName: string) => {
    try {
      // Find the file in buktiFisikMap (now an array of files per key)
      let deletedBuktiFisik: IKDBuktiFisik | undefined;
      ...
      if (res.data?.success) {
        // Langsung update local state: hapus file dari array
        setBuktiFisikMap((prev) => {
          const newMap = { ...prev };
          if (newMap[key]) {
            const updatedFiles = newMap[key].filter((bf) => bf.id !== buktiFisikId);
            if (updatedFiles.length === 0) {
              // Jika tidak ada file lagi, hapus key dan reset skor
              delete newMap[key];
              ...
              // Jika masih ada file, update array
        ...
        // Reset skor ke 0 di local state jika tidak ada file lagi
        const remainingFiles = buktiFisikMap[key]?.filter((bf) => bf.id !== buktiFisikId) || [];
  ...
  return (
    <RekapIKDBase
      title="Rekap IKD - SDM"
      description="Rekap Indikator Kinerja Dosen untuk SDM"
    >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Kelola rekap IKD untuk unit kerja SDM
            </p>
          </div>
        </div>
                                  ...
                                  return (
                                    <React.Fragment key={pedoman.id}>
                                      ...
                                      <td className="px-2 pr-4 py-4 text-center align-middle">
                                        {buktiFisikFiles.length > 0 && !savedKeys.has(key) ? (
                                          <button
                                            onClick={() => handleSimpanClick(key)}
                                      ...
                                      <td className="px-2 pr-4 py-4 text-center align-middle">
                                        <input
                                          ...
                                          disabled={buktiFisikFiles.length === 0 || !savedKeys.has(key)}
                                          className={`w-16 px-1.5 py-1 text-sm text-center border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent ${buktiFisikFiles.length === 0 || !savedKeys.has(key)
              ...
              >
                ...
                <div>
                  <p className="mb-6 text-gray-500 dark:text-gray-400">
                    Apakah Anda yakin ingin menghapus file <span className="font-semibold text-gray-800 dark:text-white">"{fileToDelete.name}"</span>?
                  </p>
          ...
          {showSaveModal && keyToSave && (
            <div className="fixed inset-0 z-[100000] flex items-center justify-center">
              ...
              <motion.div
                ...
                exit={{ opacity: 0, scale: 0.95 }}
                transition={{ duration: 0.2 }}
                className="relative w-full max-w-md mx-auto bg-white dark:bg-gray-900 rounded-2xl px-6 py-6 shadow-lg z-[100001]"
                onClick={(e) => e.stopPropagation()}
              >
                <div>
                  <h2 className="text-xl font-bold text-gray-800 dark:text-white">
                ...
                <div>
                  <p className="mb-6 text-gray-500 dark:text-gray-400">
                    Setelah menekan 'Simpan', Anda tidak dapat lagi mengupload file untuk kegiatan ini. Apakah Anda yakin ingin melanjutkan?
                  </p>
                  <div className="flex justify-end gap-2 pt-2">
                    <button
                      onClick={() => {
