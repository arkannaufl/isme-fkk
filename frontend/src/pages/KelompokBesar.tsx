...
const KelompokBesar: React.FC = () => {
    ...
    try {
      ...
      if (semester) {
        ...
        setKelompokBesarData(
          kelompokResponse.data[String(mapSemesterToNumber(semester))]
        );
        const selectedIds = (
          kelompokResponse.data[String(mapSemesterToNumber(semester))] || []
      ...
      console.error(
        "Error details:",
        handleApiError(err, "Memuat data kelompok besar")
      );
  ...
  const angkatanList = [
    ...new Set(mahasiswaBySemester.map((m) => m.angkatan)),
  ...
  const handleSelectAll = () => {
    ...
    const allSelected =
      allIds.every((id) => selectedMahasiswa.includes(id)) && allIds.length > 0;
    if (allSelected) {
      setSelectedMahasiswa(
        selectedMahasiswa.filter((id) => !allIds.includes(id))
      );
    } else {
      setSelectedMahasiswa(
        Array.from(new Set([...selectedMahasiswa, ...allIds]))
      );
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (semester) {
        ...
        const kelompokResponse = await kelompokBesarApi.batchBySemester({
          semesters: [String(mapSemesterToNumber(semester))],
        });
        setKelompokBesarData(
          kelompokResponse.data[String(mapSemesterToNumber(semester))]
      ...
      console.error(
        "Error details:",
        handleApiError(error, "Menyimpan data kelompok besar")
      );
  ...
  const handleConfirmReset = async () => {
    ...
    try {
      ...
      console.error(
        "Error details:",
        handleApiError(error, "Mereset data kelompok besar")
      );
  ...
  useEffect(() => {
    ...
    setHasUnsavedChanges(
      JSON.stringify(savedIds.sort()) !==
        JSON.stringify(selectedMahasiswa.sort())
    );
      ...
      <div className="w-full mt-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div
              key={i}
              className="flex items-center gap-3 p-3 rounded-lg border bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700 animate-pulse"
            >
    ...
    <div className="w-full mt-5">
        ...
        <div className="mb-6 relative">
          <div className="flex flex-col sm:flex-row items-start justify-between mb-2">
            <h2 className="text-lg font-semibold text-brand-700 dark:text-brand-300 flex items-center gap-2 mb-5">
              Mahasiswa Terpilih
              <span className="inline-block px-2 py-0.5 text-xs rounded-full bg-brand-600 text-white">
                {selectedMahasiswaFiltered.length}
              </span>
            </h2>
            <div className="flex gap-2 my-5 sm:my-0">
              <button
                ...
                title={
                  hasSaved
                    ? "Update data mahasiswa terpilih"
                    : "Simpan data mahasiswa terpilih"
                }
              >
                {isSaving ? (
                  <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {selectedMahasiswaFiltered.map((id) => {
              const mhs = mahasiswaBySemester.find(
                (m) => m.id.toString() === id
              );
              if (!mhs) return null;
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border transition-all duration-300 ${
                    unselectingId === id ? "opacity-0 scale-95" : "opacity-100"
                  } group ${
                    mhs.is_veteran
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-brand-900 dark:text-brand-100 text-sm">
                      {mhs.name}
                    </p>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setTimeout(() => {
                        setSelectedMahasiswa(
                          selectedMahasiswa.filter((x) => x !== id)
                        );
      ...
      <div className="mb-6 w-full bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-gray-800 rounded-xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4 shadow-theme-xs">
        ...
        <div className="w-full md:w-auto flex flex-col md:flex-row md:justify-end gap-2 items-stretch md:items-center">
          ...
          {(function () {
            const allIds = filteredMahasiswa.map((m) => m.id.toString());
            const anySelected = allIds.some((id) =>
      ...
      <div className="mb-4 text-sm text-gray-600 dark:text-gray-300">
        ...
        <span className="font-bold">
          {
            selectedMahasiswaFiltered.filter((id) =>
              filteredMahasiswa.map((m) => m.id.toString()).includes(id)
            ).length
          }
        </span>
      </div>
            ...
            {filteredMahasiswa.map((mhs) => (
              <motion.div
                ...
                className={`flex items-center gap-3 p-3 rounded-lg border transition-colors duration-200
                  ${
                    selectedMahasiswa.includes(mhs.id.toString()) ||
                    pendingSelectedId === mhs.id.toString()
                      ? mhs.is_veteran
              ...
              >
                ...
                >
                  <input
                    ...
                    } transition-colors duration-150 focus:ring-2 focus:ring-brand-300 dark:focus:ring-brand-600 relative disabled:opacity-50 disabled:cursor-not-allowed`}
                    style={{ outline: "none" }}
                  />
                  {(selectedMahasiswa.includes(mhs.id.toString()) ||
                    pendingSelectedId === mhs.id.toString()) && (
        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-lg text-gray-800 dark:text-white text-center font-medium mb-6">
              Apakah Anda yakin ingin mereset pilihan mahasiswa?
            </p>
            <div className="flex items-start gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-xl p-4 mb-8">
              ...
              <div>
                ...
                <p className="text-red-500 dark:text-red-300 leading-snug">
                  Semua pilihan mahasiswa akan dihapus dan Anda harus memilih
                  ulang dari awal.
                </p>
              </div>
            </div>
