...
export default function DetailAbSenMahasiswaSeminarPleno() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // VALIDASI: Cek apakah mahasiswa terdaftar di jadwal ini
      ...
      if (foundJadwal?.kelompok_besar_antara_id) {
        ...
        try {
          ...
          if (foundJadwal.kelompok_besar && foundJadwal.kelompok_besar.semester) {
            ...
            // kelompok_besar_id bisa berupa semester number atau ID
      ...
      try {
        ...
        const absensiData = absensiResponse.data.absensi || {};
        
        // Normalisasi NIM user untuk perbandingan yang lebih robust
        const userNim = String(user.nim || '').trim();
        
        // Cari absensi dengan perbandingan case-insensitive dan trimmed
        let absenData = null;
        for (const [nim, data] of Object.entries(absensiData)) {
          const normalizedNim = String(nim || '').trim();
          if (normalizedNim.toLowerCase() === userNim.toLowerCase()) {
            absenData = data;
            break;
          }
        }
        ...
        if (absenData) {
          ...
          if (absenData.hadir === true || absenData.hadir === 1 || absenData.hadir === '1') {
            ...
            setSuccess(true); // Set success juga agar tampilan sudah hadir muncul
          } else {
