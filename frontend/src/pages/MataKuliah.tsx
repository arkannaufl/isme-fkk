...
const isSemesterMatch = (dataSemester: string | number, filterSemester: string | number) => {
  if (filterSemester === "all") return true;
  if (filterSemester === "Antara") return dataSemester === "Antara";
  return dataSemester === filterSemester || 
...
const getSemesterOptionsForActivePeriod = (activeSemesterJenis: string | null, allData: MataKuliah[]) => {
...
export default function MataKuliah() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error details:', handleApiError(error, 'Memuat data mata kuliah'));
  ...
  useEffect(() => {
    if (form.jenis === 'Non Blok' && form.tipe_non_block === 'CSR' && form.semester !== "Antara") {
      const blokDiSemester = data.filter(mk => mk.jenis === 'Blok' && isSemesterMatch(mk.semester, form.semester));
        ...
        <div className="mb-4 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-blue-600 dark:text-blue-400">
              (Filter semester menampilkan mata kuliah periode {activeSemesterJenis})
            </span>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2 w-full lg:w-auto">
            ...
            >
              ...
              {semesterOptions.map((semester) => {
                ...
                const isActivePeriod = semester === "Antara" || 
                  (Array.isArray(data) && data.find(mk => mk.semester === semester)?.periode === activeSemesterJenis);
                
                return (
                  <option key={semester} value={semester}>
                    {semester === "Antara" ? "Semester Antara" : `Semester ${semester}`}
                    {isActivePeriod && activeSemesterJenis && semester !== "Antara" ? ` (${activeSemesterJenis})` : ''}
                  </option>
                );
              })}
            </select>
              ...
              <div>
                <div className="mb-3 sm:mb-4">
                  ...
                  <input
                    ...
                    value={form.kode}
                    readOnly
                    className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 font-normal text-sm sm:text-base cursor-not-allowed"
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const blok = data.find(
                            mk => mk.jenis === 'Blok' && isSemesterMatch(mk.semester, form.semester) && mk.blok === newCsrNumber
                          );
                          const newCsr = {
