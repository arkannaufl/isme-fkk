...
export default function DosenAbsensiNonBlokNonCSRPage() {
  ...
  const fetchQrToken = useCallback(async () => {
    ...
    try {
      ...
      if (kode && jadwalId) {
        const qrData = `${window.location.origin}/mahasiswa/absensi-non-blok-non-csr/${kode}/${jadwalId}?from_qr=true`;
        setQrCodeData(qrData);
      }
    } finally {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      // Judul Laporan
      const titleRow = worksheet.addRow(['LAPORAN HASIL ABSENSI NON-BLOK NON-CSR']);
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.setFont("times", "bold");
      yPos = addText("LAPORAN HASIL ABSENSI NON-BLOK NON-CSR", 105, yPos, { align: "center" });
      yPos += 20;
    ...
    <div className="w-full mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90 mb-2">
          Absensi Non-Blok Non-CSR
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Kelola absensi mahasiswa untuk non-blok non-CSR
        </p>
      </div>
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 text-xs text-gray-600 dark:text-gray-400">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Export Excel:</span> Ekspor data absensi non-blok non-CSR ke file Excel dengan format terstruktur dan informasi lengkap.
                </div>
              </div>
              <div className="flex items-start gap-2">
                <FontAwesomeIcon icon={faFilePdf} className="w-4 h-4 text-red-600 dark:text-red-400 mt-0.5" />
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Export PDF:</span> Ekspor data absensi non-blok non-CSR ke file PDF dengan format laporan resmi dan tanda tangan dosen.
                </div>
              </div>
            </div>
          </div>
