...
export default function DosenAbsensiPraktikumAntaraPage() {
  ...
  const fetchQrToken = useCallback(async () => {
    ...
    try {
      ...
      const qrData = `${window.location.origin}/mahasiswa/absensi-praktikum-antara/${kode}/${jadwalId}?from_qr=true&token=${token}`;
      ...
      if (kode && jadwalId) {
        const qrData = `${window.location.origin}/mahasiswa/absensi-praktikum-antara/${kode}/${jadwalId}?from_qr=true`;
        setQrCodeData(qrData);
      }
    } finally {
  ...
  useEffect(() => {
    ...
    const expectedPath = `/absensi-praktikum-antara/${kode}/${jadwalId}`;
    ...
    const wasOnAbsensiPage = previousLocationRef.current === expectedPath || 
                             previousLocationRef.current.startsWith('/absensi-praktikum-antara/');
    const isStillOnAbsensiPage = currentPath === expectedPath || 
                                  currentPath.startsWith('/absensi-praktikum-antara/');
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('Absensi Praktikum Antara');
      ...
      // Judul Laporan
      const titleRow = worksheet.addRow(['LAPORAN HASIL ABSENSI PRAKTIKUM ANTARA']);
      ...
      const filename = `Absensi_Praktikum_Antara_${jadwalDetail.mata_kuliah_kode}_${tanggalFile}`;
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.setFont("times", "bold");
      yPos = addText("LAPORAN HASIL ABSENSI PRAKTIKUM ANTARA", 105, yPos, { align: "center" });
      yPos += 20;
    ...
    <div className="w-full mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90 mb-2">
            Absensi Praktikum Antara
          </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Kelola absensi mahasiswa untuk praktikum antara
        </p>
      </div>
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 text-xs text-gray-600 dark:text-gray-400">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Export Excel:</span> Ekspor data absensi praktikum antara ke file Excel dengan format terstruktur dan informasi lengkap.
                </div>
              </div>
              <div className="flex items-start gap-2">
                <FontAwesomeIcon icon={faFilePdf} className="w-4 h-4 text-red-600 dark:text-red-400 mt-0.5" />
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Export PDF:</span> Ekspor data absensi praktikum antara ke file PDF dengan format laporan resmi dan tanda tangan dosen.
                </div>
              </div>
            </div>
          </div>
