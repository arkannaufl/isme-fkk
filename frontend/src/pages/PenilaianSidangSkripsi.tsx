...
const PenilaianSidangSkripsi = () => {
  ...
  const fetchAttendance = useCallback(async (mahasiswaId: number) => {
    ...
    try {
      // Gunakan endpoint yang sama dengan DosenAbsensiNonBlokNonCSRPage untuk konsistensi
      // Endpoint ini return data absensi untuk semua mahasiswa di jadwal ini, tapi kita filter
      // Karena tidak ada endpoint spesifik per mahasiswa yang public/mudah diakses dari sini
      const response = await api.get(`/non-blok-non-csr/jadwal/${id}`); // Cek kode jadwal dulu jika perlu, tapi di sini kita punya ID

      // Karena struktur API agak kompleks (butuh kode jadwal untuk endpoint absensi),
      // Kita coba endpoint yang lebih direct jika ada, atau gunakan endpoint mahasiswa keabsenan
      // Alternatif: gunakan endpoint detail jadwal yang mungkin sudah include absensi

      // Let's try to get code from jadwal detail if not available
      let kodeJadwal = jadwal?.mata_kuliah_kode;
      if (!kodeJadwal) {
        const jadwalRes = await api.get(`/jadwal-non-blok-non-csr/${id}`);
        kodeJadwal = jadwalRes.data.data.mata_kuliah_kode; // Adjust field name if necessary
      }
  ...
  const handleAttendanceChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Jika sudah hadir/checked, prevent uncheck (sesuai request: "saat sudah di simpan maka checkbox itu akan terdisabled")
    // Tapi logic disabled di UI mungkin lebih tepat.
    // User request: "checkbox hadir gitu... saat sudah di simpan maka checkbox itu akan terdisabled"
    // Jadi user check -> save -> disable.

    const isChecked = e.target.checked;
    if (!isChecked) return; // Prevent uncheck if we interpret "disabled after save" strictly, but usually we allow change until disabled.
    // Actually if it is disabled, onChange won't fire.
    ...
    try {
      ...
      if (!kodeJadwal) {
        alert("Gagal mengambil kode jadwal.");
        return;
      }
      ...
      const payload = { absensi: [{ mahasiswa_nim: mahasiswa.nim, hadir: true, catatan: '' }] };
      ...
      alert("Gagal menyimpan absensi data.");
      // Revert check if failed? Checkbox usually controlled by state?
      // If controlled:
      // setAttendanceStatus(false); 
    } finally {
      setLoadingAttendance(false);
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
                    <div className="flex items-center gap-3 sm:gap-4">
                      ...
                      {/* Absensi Checkbox (Hanya untuk Dosen Terlibat) */}
                      {isDosenTerlibat() && (
                        <div className="flex items-center gap-2">
                          <label className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition cursor-pointer select-none ${attendanceStatus
                            ? "bg-green-50 border-green-200 text-green-700 dark:bg-green-900/20 dark:border-green-800 dark:text-green-400"
                            : "bg-gray-50 border-gray-200 text-gray-600 dark:bg-gray-700/50 dark:border-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                            } ${loadingAttendance ? "opacity-70 cursor-wait" : ""}`}>
                            <input
                              type="checkbox"
                              className="w-4 h-4 rounded text-brand-600 focus:ring-brand-500 border-gray-300 dark:border-gray-600"
                              checked={attendanceStatus}
                              onChange={handleAttendanceChange}
                              disabled={attendanceStatus || loadingAttendance} // Disabled if already checked or loading
                            />
                            <span className="text-sm font-medium">Mahasiswa Hadir</span>
                            {loadingAttendance && (
                              <FontAwesomeIcon icon={faExclamationCircle} className="animate-spin text-xs" /> // Re-using icon for loader if spinner not imported, or just generic
                            )}
                          </label>
                        </div>
                      )}
                ...
                <div>
                  <div className="mb-3 sm:mb-4">
                    <p className="text-gray-600 dark:text-gray-300 mb-4">
                      Apakah Anda yakin ingin mem-finalize nilai sidang skripsi untuk mahasiswa ini? Tindakan ini tidak dapat dibatalkan.
                    </p>
                  </div>
          ...
          <div className="fixed inset-0 z-[100000] flex items-center justify-center">
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between pb-4 sm:pb-6">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-white">
                    Batalkan Finalize Nilai Sidang Skripsi
                  </h2>
                </div>
                ...
                <div>
                  <div className="mb-3 sm:mb-4">
                    <p className="text-gray-600 dark:text-gray-300 mb-4">
                      Apakah Anda yakin ingin membatalkan finalize nilai sidang skripsi untuk mahasiswa ini? Ini akan memungkinkan perubahan nilai kembali.
                    </p>
                  </div>
