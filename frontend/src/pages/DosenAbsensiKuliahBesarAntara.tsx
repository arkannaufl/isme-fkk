...
export default function DosenAbsensiKuliahBesarAntaraPage() {
  ...
  const fetchQrToken = useCallback(async () => {
    ...
    try {
      ...
      const qrData = `${window.location.origin}/mahasiswa/absensi-kuliah-besar-antara/${kode}/${jadwalId}?from_qr=true&token=${token}`;
      ...
      if (kode && jadwalId) {
        const qrData = `${window.location.origin}/mahasiswa/absensi-kuliah-besar-antara/${kode}/${jadwalId}?from_qr=true`;
        setQrCodeData(qrData);
      }
    } finally {
  ...
  useEffect(() => {
    ...
    const expectedPath = `/absensi-kuliah-besar-antara/${kode}/${jadwalId}`;
    ...
    const wasOnAbsensiPage = previousLocationRef.current === expectedPath || 
                             previousLocationRef.current.startsWith('/absensi-kuliah-besar-antara/');
    const isStillOnAbsensiPage = currentPath === expectedPath || 
                                  currentPath.startsWith('/absensi-kuliah-besar-antara/');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Jika menggunakan dosen_ids (array), fetch data dosen lengkap
      if (foundJadwal.dosen_ids && Array.isArray(foundJadwal.dosen_ids) && foundJadwal.dosen_ids.length > 0) {
        try {
          // Fetch data dosen pertama (atau dosen yang sedang login)
          const user = getUser();
          const dosenIdToFetch = user?.id && foundJadwal.dosen_ids.includes(user.id) 
            ? user.id 
            : foundJadwal.dosen_ids[0];
          
          const dosenResponse = await api.get(`/users/${dosenIdToFetch}`);
          foundJadwal.dosen = dosenResponse.data;
        } catch (err) {
          console.error('Error fetching dosen data:', err);
        }
      } else if (foundJadwal.dosen_id && (!foundJadwal.dosen || !foundJadwal.dosen.signature_image)) {
        // Jika menggunakan dosen_id (single) tapi signature_image belum ada, fetch ulang
        try {
          const dosenResponse = await api.get(`/users/${foundJadwal.dosen_id}`);
          foundJadwal.dosen = dosenResponse.data;
        } catch (err) {
          console.error('Error fetching dosen data:', err);
        }
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('Absensi Kuliah Besar Antara');
      ...
      const filename = `Absensi_Kuliah_Besar_Antara_${jadwalDetail.mata_kuliah_kode}_${tanggalFile}`;
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.setFont("times", "bold");
      yPos = addText("LAPORAN HASIL ABSENSI KULIAH BESAR ANTARA", 105, yPos, { align: "center" });
      yPos += 20;
    ...
    <div className="w-full mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90 mb-2">
          Absensi Kuliah Besar Antara
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          Kelola absensi mahasiswa untuk kuliah besar antara
        </p>
      </div>
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 text-xs text-gray-600 dark:text-gray-400">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Export Excel:</span> Ekspor data absensi kuliah besar antara ke file Excel dengan format terstruktur dan informasi lengkap.
                </div>
              </div>
              <div className="flex items-start gap-2">
                <FontAwesomeIcon icon={faFilePdf} className="w-4 h-4 text-red-600 dark:text-red-400 mt-0.5" />
                <div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">Export PDF:</span> Ekspor data absensi kuliah besar antara ke file PDF dengan format laporan resmi dan tanda tangan dosen.
                </div>
              </div>
            </div>
          </div>
