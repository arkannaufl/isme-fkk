...
export default function DosenAbsensiNonBlokNonCSRAntaraPage() {
  ...
  const fetchQrToken = useCallback(async () => {
    ...
    try {
      ...
      const qrData = `${window.location.origin}/mahasiswa/absensi-non-blok-non-csr-antara/${kode}/${jadwalId}?from_qr=true&token=${token}`;
      ...
      if (kode && jadwalId) {
        const qrData = `${window.location.origin}/mahasiswa/absensi-non-blok-non-csr-antara/${kode}/${jadwalId}?from_qr=true`;
        setQrCodeData(qrData);
      }
    } finally {
  ...
  useEffect(() => {
    ...
    const expectedPath = `/absensi-non-blok-non-csr-antara/${kode}/${jadwalId}`;
    ...
    const wasOnAbsensiPage = previousLocationRef.current === expectedPath || 
                             previousLocationRef.current.startsWith('/absensi-non-blok-non-csr-antara/');
    const isStillOnAbsensiPage = currentPath === expectedPath || 
                                  currentPath.startsWith('/absensi-non-blok-non-csr-antara/');
  ...
  useEffect(() => {
    return () => {
      ...
      if (qrEnabled && kode && jadwalId && !isDisablingOnUnmountRef.current) {
        ...
        api.put(`/non-blok-non-csr/${kode}/jadwal/${jadwalId}/toggle-qr`)
          .then(() => {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('Absensi Non-Blok Non-CSR Antara');
      ...
      // Judul Laporan
      const titleRow = worksheet.addRow(['LAPORAN HASIL ABSENSI NON-BLOK NON-CSR ANTARA']);
      ...
      const filename = `Absensi_Non_Blok_Non_CSR_Antara_${jadwalDetail.mata_kuliah_kode}_${tanggalFile}`;
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.setFont("times", "bold");
      yPos = addText("LAPORAN HASIL ABSENSI NON-BLOK NON-CSR ANTARA", 105, yPos, { align: "center" });
      yPos += 20;
