...
interface JadwalDetail {
  ...
  komentator_list?: { id: number; name: string; nid?: string }[];
...
interface HasilSeminarProposal {
  id?: number;
  jadwal_id: number;
  mahasiswa_id: number;
...
const ASPEK_PENILAIAN = [
  {
    ...
    subKriteria: [
      "Penggunaan Waktu",
      "Kejelasan Penyajian",
      "Efektifitas, Pemakaian alat Audio Visual",
    ],
  },
  {
  ...
  },
  {
    ...
    bobot: 3,
    subKriteria: [
      "Mampu menjelaskan rencana penelitian",
      "Mampu mempertahankan rencana penelitian",
      "Penguasaan materi",
...
const PenilaianSeminarProposal = () => {
  ...
  // State untuk Hasil Seminar (Keputusan Moderator)
  ...
  const [hasilData, setHasilData] = useState<Record<number, HasilSeminarProposal>>({});
  ...
  // Check if current user is involved dosen (pembimbing, komentator, or penguji)
  ...
  // Check if current user is Moderator/Pembimbing
  const isModerator = (): boolean => {
    if (!jadwal?.dosen_role) {
      return false;
    }
    const role = jadwal.dosen_role.toLowerCase();
    return role.includes("moderator") || role.includes("pembimbing");
  };

  // Determine peran penguji based on dosen_role
  const getPeranPenguji = (): string => {
    if (!jadwal?.dosen_role) return "moderator";
    const role = jadwal.dosen_role.toLowerCase();
    if (role.includes("moderator") || role.includes("pembimbing")) return "moderator";
    if (role.includes("komentator 1") || role.includes("materi")) return "komentator_1";
    if (role.includes("komentator 2") || role.includes("metlit") || role.includes("metodologi")) return "komentator_2";
    return "moderator";
  };
  ...
  const fetchPenilaian = useCallback(async () => {
    try {
      const response = await api.get(`/penilaian-seminar-proposal/jadwal/${id}`);
  ...
  const fetchHasilSeminar = useCallback(async () => {
    try {
      const response = await api.get(`/hasil-seminar-proposal/jadwal/${id}`);
      const hasilArray = response.data.data || [];
      const hasilMap: Record<number, HasilSeminarProposal> = {};
      hasilArray.forEach((h: HasilSeminarProposal) => {
        hasilMap[h.mahasiswa_id] = h;
      });
      setHasilData(hasilMap);
    } catch (error) {
      // Error fetching hasil seminar
    }
  }, [id]);

  // Fetch Attendance Status for selected Mahasiswa
  const fetchAttendance = useCallback(async (mahasiswaId: number) => {
    if (!id) return;
    setLoadingAttendance(true);
    try {
      // Get jadwal code if not available
      let kodeJadwal = jadwal?.mata_kuliah_kode;
      if (!kodeJadwal) {
        const jadwalRes = await api.get(`/jadwal-non-blok-non-csr/${id}`);
        kodeJadwal = jadwalRes.data.data.mata_kuliah_kode;
      }

      if (kodeJadwal) {
        const absensiResponse = await api.get(`/non-blok-non-csr/${kodeJadwal}/jadwal/${id}/absensi`);
        const mahasiswa = jadwal?.mahasiswa_list?.find(m => m.id === mahasiswaId);
        if (mahasiswa && absensiResponse.data?.absensi && absensiResponse.data.absensi[mahasiswa.nim]) {
          setAttendanceStatus(absensiResponse.data.absensi[mahasiswa.nim].hadir || false);
        } else {
  ...
  }, [id, hasUnsavedChanges, hasUnsavedHasil, fetchPenilaian, fetchHasilSeminar]);
  ...
  const handleSelectMahasiswa = async (mahasiswaId: number) => {
    ...
    try {
      const response = await api.get(`/penilaian-seminar-proposal/jadwal/${id}/mahasiswa/${mahasiswaId}/my`);
  ...
  const handleAttendanceChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedMahasiswa || !id || !jadwal) return;

    const isChecked = e.target.checked;
    if (!isChecked) return; // Prevent uncheck if desired, or allow toggle
    ...
    try {
      ...
      if (kodeJadwal && selectedMahasiswaData) {
        const payload = {
          absensi: [
            {
              mahasiswa_nim: selectedMahasiswaData.nim,
              hadir: isChecked,
              catatan: "Hadir Seminar Proposal"
            }
          ]
        };

        await api.post(`/non-blok-non-csr/${kodeJadwal}/jadwal/${id}/absensi`, payload);

        // Update local state
        setAttendanceStatus(isChecked);

        // Show success notification (optional sidebar update)
        // fetchAttendance(selectedMahasiswa); // Reload to confirm
      }
    } catch (error) {
  ...
  const handleSavePenilaian = async () => {
    ...
    try {
      await api.post("/penilaian-seminar-proposal", {
  ...
  const handleSaveHasil = async () => {
    ...
    try {
      await api.post("/hasil-seminar-proposal", {
        jadwal_id: parseInt(id!),
        mahasiswa_id: selectedMahasiswa,
  ...
  const handleFinalize = async () => {
    ...
    try {
      await api.post("/hasil-seminar-proposal/finalize", {
        jadwal_id: parseInt(id!),
        mahasiswa_id: selectedMahasiswa,
      });
  ...
  const handleUnfinalize = async () => {
    ...
    try {
      await api.post("/hasil-seminar-proposal/unfinalize", {
        jadwal_id: parseInt(id!),
        mahasiswa_id: selectedMahasiswa,
      });
  ...
  const allPengujiHaveSubmitted = (): boolean => {
    ...
    // Add moderator/pembimbing
    if (jadwal.pembimbing?.id) {
      expectedPengujiIds.push(jadwal.pembimbing.id);
    }

    // Add komentator
    if (jadwal.komentator_list) {
      jadwal.komentator_list.forEach((k) => {
        if (k.id) {
          expectedPengujiIds.push(k.id);
        }
      });
    }
  ...
  const generateBeritaAcaraPDF = async () => {
    try {
      ...
      // Fetch tanda tangan untuk komentator_list
      if (jadwal.komentator_list) {
        const komentatorPromises = jadwal.komentator_list.map(async (k) => {
          const signature = await fetchPengujiSignature(k.id);
          return { pengujiId: k.id, signature };
        });
        const komentatorResults = await Promise.all(komentatorPromises);
        komentatorResults.forEach(({ pengujiId, signature }) => {
          if (signature) {
            pengujiSignatures[pengujiId] = signature;
          }
        });
      ...
      if (jadwal.pembimbing) {
        allPenguji.push({ name: jadwal.pembimbing.name, role: "Moderator" });
      }
      jadwal.komentator_list?.forEach((k, idx) => {
        allPenguji.push({ name: k.name, role: `Komentator ${idx + 1} (${idx === 0 ? "Materi" : "Metlit"})` });
      });
      ...
      const paragraph1 = `Pada hari ini ${tanggalFormatted}, telah diselenggarakan Seminar Proposal terhadap Peserta didik Program Studi Kedokteran Universitas Muhammadiyah Jakarta, atas nama :`;
      ...
      // Tim Penguji
      doc.text("Berdasarkan keputusan moderator/Ketua Penguji dan penguji yang terdiri dari :", margin, yPos);
      yPos += 6;
      doc.text("Moderator", margin, yPos);
      doc.text(`: ${jadwal.pembimbing?.name || "-"}`, margin + 35, yPos);
      yPos += 6;
      doc.text("Anggota", margin, yPos);
      jadwal.komentator_list?.forEach((k, idx) => {
        const prefix = idx === 0 ? ": " : "  ";
        doc.text(`${prefix}Komentator ${idx + 1} (${idx === 0 ? "Materi" : "Metlit"}) ${k.name}`, margin + 35, yPos);
        yPos += 5;
      });
      yPos += 10;
      ...
      allPenguji.forEach((p, idx) => {
        ...
        if (p.role === "Moderator" && jadwal.pembimbing?.id) {
          pengujiId = jadwal.pembimbing.id;
        } else if (p.role.startsWith("Komentator")) {
          // Cari komentator berdasarkan nama
          const komentatorIdx = jadwal.komentator_list?.findIndex(k => k.name === p.name);
          if (komentatorIdx !== undefined && komentatorIdx >= 0 && jadwal.komentator_list) {
            pengujiId = jadwal.komentator_list[komentatorIdx].id;
          }
        }
      ...
      // Tanggal dan tanda tangan moderator (pakai tanggal jadwal)
      ...
      yPos += 6;
      doc.text("Moderator/Ketua Penguji,", pageWidth - margin - 60, yPos);
      yPos += 10;
      ...
      const aspekData = [
        {
          ...
          sub: ["Penggunaan Waktu", "Kejelasan Penyajian", "Efektifitas, Pemakaian alat Audio Visual"],
          bobot: 2,
          key: "nilai_penyajian_lisan",
        },
        {
          ...
          sub: ["Sesuai Kaidah Ilmiah", "Penggunaan Bahasa Indonesia yang baik dan benar", "Susunan Tata Bahasa"],
          bobot: 1,
          key: "nilai_sistematika_penulisan",
        },
        {
          no: "3.",
          title: "Isi Tulisan",
          sub: ["Latar belakang", "Tujuan", "Tinjauan pustaka", "Kerangka teori", "Kerangka konsep", "Definisi Operasional (DO)", "Desain penelitian", "Metode pengambilan data", "Rencana analisis data", "Referensi"],
          bobot: 3,
          key: "nilai_isi_tulisan",
        },
        {
        ...
        },
        {
          ...
          sub: ["Mampu menjelaskan rencana penelitian", "Mampu mempertahankan rencana penelitian", "Penguasaan materi", "Ketepatan menjawab pertanyaan"],
          bobot: 3,
          key: "nilai_tanya_jawab",
        },
      ];
      ...
      if (pengujiList.length === 0) {
        ...
        doc.setFontSize(14);
        doc.setFont("times", "bold");
        doc.text("FORMULIR PENILAIAN SEMINAR PROPOSAL", pageWidth / 2, yPos + 5, { align: "center" });
        yPos += 20;
        doc.setFont("times", "normal");
      ...
      pengujiList.forEach((pengujiNilai) => {
        ...
        doc.setFontSize(14);
        doc.setFont("times", "bold");
        yPos = addText("FORMULIR PENILAIAN SEMINAR PROPOSAL", pageWidth / 2, yPos + 5, { align: "center" });
        yPos += 15;
      ...
      doc.setFontSize(14);
      doc.setFont("times", "bold");
      yPos = addText("NILAI SEMINAR PROPOSAL", pageWidth / 2, yPos + 5, { align: "center" });
      yPos += 15;
      ...
      doc.text("Moderator /Penguji", nilaiTableX + 30, yPos + 5);
      ...
      // Tanda tangan
      doc.text("Moderator Ketua penguji", pageWidth - margin - 50, yPos);
      yPos += 10;
      ...
      // Save PDF
      const fileName = `Berita_Acara_Sempro_${mahasiswa.nim}_${jadwal.tanggal.split("-").reverse().join("-")}.pdf`;
      doc.save(fileName);
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            {/* Sidebar Skeleton */}
            <div className="w-80 lg:sticky lg:top-24 self-start hidden lg:block">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                ...
                <div className="h-10 bg-gray-200 dark:bg-gray-700 rounded-lg mb-3 animate-pulse"></div>
  ...
  return (
    <>
      <PageMeta title="Penilaian Seminar Proposal | ISME" description="Form Penilaian Seminar Proposal" />
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <button
                  onClick={() => navigate(`/bimbingan-akhir/seminar-proposal/${id}`)}
                ...
                <div>
                  <h1 className="text-xl font-bold text-gray-900 dark:text-white">Penilaian Seminar Proposal</h1>
                    ...
                    .map((mahasiswa, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-xl transition ${isSelected
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <span
                                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${isSelected
            ...
            <div className="flex-1 min-w-0">
              {selectedMahasiswa && selectedMahasiswaData ? (
                <div className="space-y-6">
                  ...
                  {/* Attendance Checkbox - Only for Dosen Terlibat */}
                  {isDosenTerlibat() && (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${attendanceStatus ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'}`}>
                            <FontAwesomeIcon icon={faCheckCircle} className="text-lg" />
                          </div>
                          <div>
                            <h3 className="text-base font-semibold text-gray-900 dark:text-white">Absensi Mahasiswa</h3>
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {attendanceStatus
                                ? "Mahasiswa tercatat hadir dalam sidang ini"
                                : "Konfirmasi kehadiran mahasiswa"}
                            </p>
                          </div>
                        </div>

                        <label className={`flex items-center gap-3 px-4 py-2 rounded-lg border transition cursor-pointer select-none ${attendanceStatus
                            ? "bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800"
                            : "bg-white border-gray-200 hover:border-brand-300 dark:bg-gray-800 dark:border-gray-700"
                          }`}>
                          <div className="relative flex items-center">
                            <input
                              type="checkbox"
                              className="w-5 h-5 rounded border-gray-300 text-brand-600 focus:ring-brand-500 disabled:opacity-50 disabled:cursor-not-allowed"
                              checked={attendanceStatus}
                              onChange={handleAttendanceChange}
                              disabled={loadingAttendance || attendanceStatus} // Disable if loading or already present (if requirement is one-way)
                            />
                            {loadingAttendance && (
                              <div className="absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-800 opacity-80">
                                <div className="w-3 h-3 border-2 border-brand-500 border-t-transparent rounded-full animate-spin"></div>
                              </div>
                            )}
                          </div>
                          <span className={`text-sm font-medium ${attendanceStatus ? 'text-green-700 dark:text-green-300' : 'text-gray-700 dark:text-gray-300'}`}>
                            {attendanceStatus ? "Mahasiswa Hadir" : "Tandai Hadir"}
                          </span>
                        </label>
                      </div>
                    </div>
                      ...
                      <div className="space-y-4">
                            ...
                            <div className="ml-8 sm:ml-10">
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className={`w-28 sm:w-36 px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-base sm:text-lg font-medium focus:ring-2 focus:ring-brand-500 focus:border-transparent transition ${!canEditPenilaian() ? "opacity-60 cursor-not-allowed" : ""
                                    }`}
                                  placeholder="0.00"
                                />
                        ...
                        <div className="space-y-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                          ...
                          <textarea
                            ...
                            rows={3}
                            disabled={!canEditPenilaian()}
                            className={`w-full px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-brand-500 focus:border-transparent transition ${!canEditPenilaian() ? "opacity-60 cursor-not-allowed" : ""
                              }`}
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                          <button
                            ...
                            className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-semibold text-sm sm:text-base transition ${hasUnsavedChanges
                          ...
                          {saveMessage && (
                            <span
                              className={`text-xs sm:text-sm text-center sm:text-left ${saveMessage.type === "success" ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                                }`}
                            >
                              {saveMessage.text}
                            </span>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
                      ...
                      <div className="space-y-3 mb-4">
                        {selectedPenilaian.nilai_per_penguji.map((np, npIdx) => (
                          <div
                            key={npIdx}
                            className={`rounded-xl border p-3 sm:p-4 ${np.penguji_id === currentUserId
                          ...
                          >
                            ...
                            <div className="flex items-start sm:items-center justify-between gap-2 mb-3">
                              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 ${np.penguji_id === currentUserId
                                  ? "bg-brand-100 dark:bg-brand-900/50"
                                  : "bg-gray-200 dark:bg-gray-600"
                                  }`}>
                                  <span className={`text-xs sm:text-sm font-bold ${np.penguji_id === currentUserId
                  ...
                  {/* Form Keputusan Moderator - Hanya tampil jika user adalah Moderator */}
                  {isModerator() && (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                      ...
                      <div className="bg-gradient-to-r from-brand-500 to-brand-600 px-6 py-4">
                        <h3 className="text-lg font-semibold text-white flex items-center gap-3">
                          ...
                          Keputusan Seminar Proposal
                        </h3>
                        <p className="text-sm text-white/80 mt-1">
                          Tentukan keputusan akhir sebagai Moderator/Ketua Penguji
                        </p>
                      </div>
                      ...
                      <div className="p-6 space-y-5">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            className={`w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition resize-none ${!isDosenTerlibat() || (hasilData[selectedMahasiswa]?.is_finalized && !isAdmin()) || !allPengujiHaveSubmitted()
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            {[
                              ...
                              <label
                                key={option.value}
                                className={`relative flex flex-col items-center p-4 rounded-xl border-2 transition-all ${!isDosenTerlibat() || (hasilData[selectedMahasiswa]?.is_finalized && !isAdmin()) || !allPengujiHaveSubmitted()
                                  ? "opacity-60 cursor-not-allowed"
                                  : "cursor-pointer"
                              ...
                              >
                                ...
                                />
                                <div
                                  className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold mb-2 ${hasilForm.keputusan === option.value
                                    ? "bg-brand-500 text-white"
                                ...
                                <span className={`text-sm font-semibold text-center ${hasilForm.keputusan === option.value
                        ...
                        {hasilForm.keputusan === "lulus_dengan_perbaikan" && (
                          <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800">
                            ...
                            <textarea
                              ...
                              rows={4}
                              disabled={!isDosenTerlibat() || (hasilData[selectedMahasiswa]?.is_finalized && !isAdmin()) || !allPengujiHaveSubmitted()}
                              className={`w-full px-4 py-3 rounded-lg border border-amber-200 dark:border-amber-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition resize-none ${!isDosenTerlibat() || (hasilData[selectedMahasiswa]?.is_finalized && !isAdmin()) || !allPengujiHaveSubmitted()
                                ? "opacity-60 cursor-not-allowed"
                                : ""
                                }`}
                        ...
                        <div className="pt-4 border-t border-gray-100 dark:border-gray-700 space-y-3">
                          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            <button
                              ...
                              className={`flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-semibold transition shadow-sm ${hasUnsavedHasil
                                ? "bg-amber-500 text-white hover:bg-amber-600"
                                : "bg-brand-500 text-white hover:bg-brand-600"
                                } disabled:opacity-50 disabled:cursor-not-allowed`}
                          ...
                          {hasilMessage && (
                            <div className="flex items-center justify-start">
                              <span
                                className={`text-sm font-medium ${hasilMessage.type === "success" ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                                  }`}
                              >
                                {hasilMessage.text}
                              </span>
                  ...
                  {!isModerator() && hasilData[selectedMahasiswa] && (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${hasilData[selectedMahasiswa].keputusan === "tidak_lulus"
                  ...
                  {/* Info belum ada keputusan - Untuk non-moderator */}
                  {!isModerator() && !hasilData[selectedMahasiswa] && (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">Menunggu keputusan dari Moderator/Ketua Penguji</p>
                    </div>
                  )}
                </div>
              ) : (
              ...
              <div>
                <div className="flex items-center justify-between pb-4 sm:pb-6">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-white">
                    Finalize Nilai Seminar Proposal
                  </h2>
                </div>
                ...
                <div>
                  <div className="mb-3 sm:mb-4">
                    <p className="text-gray-600 dark:text-gray-300 mb-4">
                      Apakah Anda yakin ingin mem-finalize nilai seminar proposal untuk mahasiswa ini? Tindakan ini tidak dapat dibatalkan.
                    </p>
                  </div>
                  ...
                  <div className="flex justify-end gap-2 pt-2 relative z-20">
                    ...
                    <button
                      ...
                      disabled={finalizing}
                      className="px-3 sm:px-4 py-2 rounded-lg bg-green-600 text-white text-xs sm:text-sm font-medium hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 ease-in-out"
                    >
          ...
          <div className="fixed inset-0 z-[100000] flex items-center justify-center">
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between pb-4 sm:pb-6">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-white">
                    Batalkan Finalize Nilai Seminar Proposal
                  </h2>
                </div>
                ...
                <div>
                  <div className="mb-3 sm:mb-4">
                    <p className="text-gray-600 dark:text-gray-300 mb-4">
                      Apakah Anda yakin ingin membatalkan finalize nilai seminar proposal untuk mahasiswa ini? Ini akan memungkinkan perubahan nilai kembali.
                    </p>
                  </div>
                  ...
                  <div className="flex justify-end gap-2 pt-2 relative z-20">
                    ...
                    <button
                      ...
                      disabled={finalizing}
                      className="px-3 sm:px-4 py-2 rounded-lg bg-red-600 text-white text-xs sm:text-sm font-medium hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 ease-in-out"
                    >
...
export default PenilaianSeminarProposal;
